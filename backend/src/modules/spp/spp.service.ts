...
export class SPPService {
  ...
  static async getAllSPPRequests(params: SPPQueryParams = {}): Promise<{
    ...
    let baseQuery = `
      SELECT
        sr.id, sr.spp_number, DATE(sr.request_date) as request_date, sr.requested_by, 
  ...
  static async getSPPRequestById(id: number): Promise<SPPRequestWithItems | null> {
    ...
    const [sppRows] = await pool.query<RowDataPacket[]>(
      `SELECT 
        id, spp_number, DATE(request_date) as request_date, requested_by, status, notes, created_by, created_at, updated_at
    ...
    const [itemRows] = await pool.query<RowDataPacket[]>(
      `SELECT
        ...
        si.description, si.unit, si.request_qty, si.receive_qty, 
        si.remaining_qty, si.request_status, DATE(si.date_req) as date_req, 
        si.item_status, si.created_at,
        m.id as material_id,
        m.material_code,
  ...
  static async createSPPRequest(data: CreateSPPRequestDTO): Promise<SPPRequestWithItems> {
    ...
    try {
      ...
      // Create SPP request
      const sql1 = 'INSERT INTO spp_requests (spp_number, request_date, requested_by, status, notes, created_by) VALUES (?, ?, ?, ?, ?, ?)';
      const values1 = [
        spp_number,
        data.request_date,
        data.requested_by,
      ...
      if (data.items && data.items.length > 0) {
        for (let i = 0; i < data.items.length; i++) {
          ...
          const sql = 'INSERT INTO spp_items (spp_id, material_id, list_item_number, list_item, description, unit, request_qty, date_req) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
          const values = [
            sppId,
          ...
          console.log(`[SPP Create Item] spp_id=${sppId}, item=${i+1}`, JSON.stringify(values));
  ...
  static async addSPPItem(
    ...
    await pool.query(
      'INSERT INTO spp_items (spp_id, material_id, list_item_number, list_item, description, unit, request_qty, date_req) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [
        sppId,
  ...
  static async approveSPP(
    ...
    try {
      ...
      if (data.approval_status === 'APPROVED' && data.approval_role === 'material_site') {
        ...
        // Update SPP status to RECEIVED
        await connection.query(
          'UPDATE spp_requests SET status = ? WHERE id = ?',
          ['RECEIVED', sppId]
        );

        // Update all items status to RECEIVED
        await connection.query(
          'UPDATE spp_items SET item_status = ? WHERE spp_id = ?',
          ['RECEIVED', sppId]
        );
      }
  ...
  private static async createInventoryFromSPP(
    ...
    for (const item of itemRows) {
      // Determine if TOOL or MATERIAL based on material type
      // For now, we'll use a simple heuristic: if material exists and has type, check the type
      let item_type: 'TOOL' | 'MATERIAL' = 'MATERIAL'; // Default to consumable
      ...
      if (item.material_id) {
        ...
        if (materialRows.length > 0) {
          const typeName = materialRows[0].type_name?.toLowerCase() || '';
          // If type contains words like "tool", "equipment", "durable" - mark as TOOL
          if (
            typeName.includes('tool') ||
            typeName.includes('equipment') ||
            typeName.includes('durable')
          ) {
            item_type = 'TOOL';
          }
        }
